...
### 如何判断文件类型
0. 文本文件只支持utf8编码。如果用户的文件是别的编码格式，需要他先转换为utf8以后再用msp进行阅读。
1. 如果文件体积大于MAX_BUF_LEN，则拒绝处理。
2. 如果头8个字节是89 50 4e 47 0d 0a 1a 0a， 则认为该文件是PNG格式，就把文件以二进制的形式一次性读入内存。
3. 如果除去开始的空格和tab字符，以<svg或者<?xml开头，则认为是svg文件。
4. 从头开始扫描，直到遇到某一个非utf8编码的字符为止，后面的内容被舍弃。如果探测的utf8字符超过0个，则认为该文件为markdown的文本文件，因为任何utf8编码的文件均为合法的markdown文件。
